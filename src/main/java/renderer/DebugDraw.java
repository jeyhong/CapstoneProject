...
public class DebugDraw {
    ...
    public static void addBox2D(Vector2f center, Vector2f dimension, float rotation, Vector3f color, int lifetime) {
        Vector2f min = new Vector2f(center).sub(new Vector2f(dimension).mul(0.5f)); //btm left
    ...
    public static void addCircle(Vector2f center, float radius, Vector3f color, int lifetime){
        Vector2f[] points = new Vector2f[20]; //segments
        int increment = 360 / points.length;
        int currAngle = 0;

        for (int i = 0; i < points.length; i++){
            Vector2f temp = new Vector2f(radius, 0);
            JMath.rotate(temp, currAngle, new Vector2f());
