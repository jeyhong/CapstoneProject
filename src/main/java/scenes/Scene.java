...
public class Scene {
    ...
    public void addGameObjToScene(GameObject go) {
        if (!isRunning) {
            gameObjects.add(go);
        } else {
    ...
    public <T extends Component> GameObject getGameObjWith(Class<T> clazz) {
        for (GameObject go : gameObjects) {
    ...
    public GameObject getGameObj(int gameObjId) {
    ...
    public GameObject getGameObj(String gameObjName) {
        Optional<GameObject> result = this.gameObjects.stream()
                .filter(gameObj -> gameObj.name.equals(gameObjName))
                .findFirst();
        return result.orElse(null);
    }
