...
public class MouseControls extends Component {
    ...
    public void editorUpdate(float dt) {
        ...
        if (holdingObject != null) {
            ...
            GameObject pickedObj = currentScene.getGameObj(gameObjectId);
            ...
            for (Integer gameObjectId : uniqueGameObjectIds) {
                GameObject pickedObj = Window.getScene().getGameObj(gameObjectId);
                if (pickedObj != null && pickedObj.getComponent(NonPickable.class) == null) {
    ...
    private boolean blockInSquare(float x, float y) {
        ...
        for (int i = 0; i < gameObjectIds.length; i++) {
            if (gameObjectIds[i] >= 0) {
                GameObject pickedObj = Window.getScene().getGameObj((int) gameObjectIds[i]);
