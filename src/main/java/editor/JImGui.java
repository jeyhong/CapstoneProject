...
public class JImGui {
    ...
    public static void drawVec2Control(String label, Vector2f vals){
        drawVec2Control(label, vals, 0.0f, defaultColumnWidth);
    }
    ...
    public static void drawVec2Control(String label, Vector2f vals, float resetVal, float columnWidth){
        ...
        Vector2f btnSize = new Vector2f(lineHeight + 3.0f, lineHeight);
        float widthEach = (ImGui.calcItemWidth() - btnSize.x * 2.0f) / 2.0f;

        ImGui.pushItemWidth(widthEach);
        ImGui.pushStyleColor(ImGuiCol.Button, 0.8f, 0.1f, 0.1f, 1.0f);
        ImGui.pushStyleColor(ImGuiCol.ButtonHovered, 0.9f, 0.2f, 0.2f, 1.0f);
        ImGui.pushStyleColor(ImGuiCol.ButtonActive, 0.8f, 0.1f, 0.1f, 1.0f);
        if(ImGui.button("X", btnSize.x, btnSize.y)){
            vals.x = resetVal;
        }
        ImGui.popStyleColor(3);

        ImGui.sameLine();
        float[] vecValsX = {vals.x};
        ImGui.dragFloat("##x", vecValsX, 0.1f);
        ImGui.popItemWidth();
        ImGui.sameLine();
        ...
        if(ImGui.button("Y", btnSize.x, btnSize.y)){
            vals.y = resetVal;
        }
        ImGui.popStyleColor(3);

        ImGui.sameLine();
        float[] vecValsY = {vals.y};
        ImGui.dragFloat("##y", vecValsY, 0.1f);
        ImGui.popItemWidth();
        ImGui.sameLine();
    ...
    public static boolean colorPick4(String label, Vector4f color){
        boolean res = false;
        ImGui.pushID(label);
